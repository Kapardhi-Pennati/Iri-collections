...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        try:
            with transaction.atomic():
                ...
                self.stdout.write(self.style.WARNING('Rolling back transaction to preserve DB state...'))
                transaction.set_rollback(True)
                self.stdout.write(self.style.SUCCESS('Schema Stress Test complete. Database restored.'))
    ...
    def run_stress_test(self):
        ...
        # Categories
        cat_start = time.time()
        categories = [Category(name=f'StressCat_{i}', slug=f'stress-cat-{i}', description='Stress test category') for i in range(100)]
        ...
        users = [User(username=f'stress_user_{i}', email=f'stress_user_{i}@test.com', full_name=f'Stress User {i}') for i in range(1000)]
        ...
        orders_in_db = list(Order.objects.filter(order_number__startswith='STRESS-ORD-'))
        ...
        q1_start = time.time()
        list(Product.objects.filter(is_active=True, price__gte=5000, stock__lte=50).select_related('category')[:50])
        q1_time = time.time() - q1_start
        self.stdout.write(f'Filtered & Paginated Products (select_related) in {q1_time:.4f}s')

        # Query 2: Aggregation (Admin Dashboard Analytics style)
        q2_start = time.time()
        revenue_metrics = Order.objects.filter(status__in=['confirmed', 'shipped', 'delivered']).aggregate(
            total_revenue=Sum('total_amount'),
            total_orders=Count('id')
        )
        q2_time = time.time() - q2_start
        self.stdout.write(f'Aggregated Revenue spanning 10k+ rows in {q2_time:.4f}s (Result: {revenue_metrics})')
        ...
        self.stdout.write(f'Calculated Top 10 Selling Products (Join + Group By) in {q3_time:.4f}s')

        # Query 4: User Order History (prefetch_related)
        q4_start = time.time()
        random_user = random.choice(users_in_db)
        user_orders = list(Order.objects.filter(user=random_user).prefetch_related('items', 'items__product'))
        q4_time = time.time() - q4_start
        self.stdout.write(f'Fetched comprehensive user order history (prefetch_related) in {q4_time:.4f}s')
